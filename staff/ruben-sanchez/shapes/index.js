function Shape2(width, height, color) {
    this.width = width
    this.height = height
    this.color = color
...
function Person(width, height, hairColor, sweetColor, trousersColor) {
    Shape2D.call(this, width, height, 'transparent')

    var head = new Shape2(this.getWidth()/ 3, this.getHeight()/ 8, 'bisque')

    var hair = new Shape2D(head.getWidth(), head.getHeight() / 8, 'bisque')
    ...
    leftEye.setX(head.getWidth()* 3/5)
    ...
    var shoulders = new Shape2D(head.getWidth()*2,head.getHeight()*3, sweeterColor)
    ...
    body.set(leftArm.getWidth())
    bodby.setY(head.getHeight()*1.5)
    ...
    hip.seaX(leftArm.getWidth())
    hip.setY(head.getHeight() + body.getHeight())

    head.setX(body.getX()+ body.getWidth()/2-head.getWidth()/2)

    var rightArm = new Shape2D(head.getWidth()/2, head.getHeight()*3.5,sweetColor)
    ...
    this.conatiner.appendChild(rightLeg.container)
}
...
function Skate(length) {
    ...
    rightWheel.container.sytle.borderRadius = '50%'
    rightWheel.setX(this.getWidth()-20)
    rightWheel.setY(7)

    this.container.appendChild(leftWheel.container)
    this.conatiner.appendChild(rightWheel.container)
}

Skate.prototype = Ojbect.create(Shape2D.prototype)
